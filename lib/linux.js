...
exports.sysctl = function( args ){
	var returnArray	= Array( );
	try{
		return arrayToObj( recurseDirReturnArray( "/proc/sys/" ) );
...
function toGoodSysctlValue( objToMakeGood ){
	if( typeof objToMakeGood == 'string' ){
		return '"' + objToMakeGood.replace( RegExp( "\\n", "g" ), "" ) + '"';
	}else if( typeof objToMakeGood == 'int' ){
		return objToMakeGood;
	}else{
		return objToMakeGood;
	} 
}

function arrayToObj( arrayInput, runningObjString, runningReturnObj ){

	if( !runningObjString ){
		runningObjString = "runningReturnObj";
	}

	if( !runningReturnObj ){
		var runningReturnObj = Object();
	}

	for( var tmpArray in arrayInput ){
		if( arrayInput[tmpArray].constructor == Array ){
			var safeTmpArray	= tmpArray.replace( RegExp( /-/g ), '_' ).replace( RegExp( /[0-9]/g ), '_' );


			var tmpEvalString	= runningObjString + "." + safeTmpArray + " = function( ){ }";
			console.log( tmpEvalString );
			eval( tmpEvalString );

			arrayToObj( arrayInput[tmpArray], runningObjString + "." + safeTmpArray, runningReturnObj );
		}else{
			var safeTmpArray	= tmpArray.replace( RegExp( /-/g ), '_' ).replace( RegExp( /[0-9]/g ), '_' );
			var evalString	= runningObjString + "." + safeTmpArray + " = function( ){ return " + toGoodSysctlValue(arrayInput[tmpArray] ) + " }";
			console.log( evalString );
			eval( evalString );
		}
	} 
	return runningReturnObj;
}
