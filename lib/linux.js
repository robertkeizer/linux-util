...
exports.sysctl = function( currentDirectory, runningObj, runningString ){
	//console.log( "DEBUG: sysctl called with dir of '" + currentDirectory + "'" );
	if( !currentDirectory ){
		currentDirectory = "/proc/sys/";
	}

	if( !runningObj ){
		runningObj	= new Object( );
	}
	...
	try{
		var dirContents	= fs.readdirSync( currentDirectory, 'utf8' );
		for( var x=0; x<dirContents.length; x++ ){
			var tmpDir	= dirContents[x];
			var safeDir	= dirContents[x].replace( RegExp( '-', 'g' ), '_' );
			var statObj	= fs.lstatSync( path.join( currentDirectory, dirContents[x] ) );
			if( statObj.isDirectory() ){
				runningObj.safeDir	= exports.sysctl( path.join( currentDirectory, tmpDir ), runningObj, runningString +"."+safeDir );
			}else{
				var tmpFileContent	= fs.readFileSync( path.join( currentDirectory, dirContents[x] ), 'utf8' );
				runningObj.__defineGetter__( runningString + "." + safeDir, function( ){
					return tmpFileContent;
				} );
			}
		}

		//console.log( "DEBUG: runningObj is now: " + sys.inspect( runningObj ) );
		return runningObj;
