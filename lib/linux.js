...
exports.interfaces	= {
	'list': iface_lib.list,
	'get_details': function( iface ){
		t_details	= iface_lib.get_details( iface );
		if( !t_details ){
			return false;
		}

		/*
		Working on getting a cleaner MAC address.. might have to re-visit the C++ code..
		mac_parts	= [ ];
		for( i=6; i<t_details.mac.length; i+=8 ){
			mac_parts.push( t_details.mac.slice( i, i+2 ) );
		}

		console.log( mac_parts );
		*/
