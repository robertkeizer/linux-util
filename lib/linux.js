...
function recursiveSysctlObjFromDir( currentDir ){
	...
	try{
		// This variable is so that the values of the files don't get overwritten by utilizing the same variable..
		var fileContentObj	= Array( );
		...
		for( var x=0; x<contentsOfDir.length; x++ ){
			...
			// The element in the directory is a file...
				...
				// This is the problem... this variable is being overwritten in the for loop.. so when the function
				// below is called it gets the last one in the directory.. 
                                fileContentObj[makeSafeName( contentsOfDir[x] )] = makeSafeContent( fs.readFileSync( path.join( currentDir, contentsOfDir[x] ), 'utf8' ) );
	//			console.log( "DEBUG: fileContentObj[" + makeSafeName( contentsOfDir[x] ) + "] is " + makeSafeContent( fs.readFileSync( path.join( currentDir, contentsOfDir[x] ), 'utf8' ) ) );
				returnObj.__defineGetter__( makeSafeName( contentsOfDir[x] ), function( ){
					return makeSafeContent( fileContentObj[makeSafeName( contentsOfDir[x] )] );
				} );
			}
		}
