...
exports.sysctl = function( args ){
	var returnArray	= Array( );
	try{
		return arrayToObj( recurseDirReturnArray( "/proc/sys/" ), "returnObj" );
...
function arrayToObj( arrayInput, runningObjString ){
	...
	for( var tmpArray in arrayInput ){
		if( arrayInput[tmpArray].constructor == Array ){
			...
			var evalString	= "returnObj." + safeTmpArray + " = [ {" + arrayToObj( arrayInput[tmpArray], runningObjString+"."+safeTmpArray ) + " } ]";
			console.log( evalString );
		}else{
			var safeTmpArray	= tmpArray.replace( RegExp( /-/g ), '_' ).replace( RegExp( /[0-9]/g ), '_' );
			var evalString	= runningObjString + "." + safeTmpArray + " = function( ){ return " + arrayInput[tmpArray].toString().replace( RegExp( "\n" ), '' ) + " }";
			console.log( evalString );
		}
	} 
	return returnObj;
