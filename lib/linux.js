	...
	try{
		...
		for( var x=0; x<contentsOfDir.length; x++ ){
			...
			// The element in the directory is a file...
				var fileContent	= makeSafeContent( fs.readFileSync( path.join( currentDir, contentsOfDir[x] ), 'utf8' ) );
				//console.log( 'DEBUG: defining getter of "' + makeSafeName( contentsOfDir[x] ) + '" "' + fileContent + '"' );
				returnObj.__defineGetter__( makeSafeName( contentsOfDir[x] ), function( ){
					return makeSafeContent( fileContent );
				} );
			}
		}
