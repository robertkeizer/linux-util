...
exports.sysctl = function( args ){
	var returnArray	= Array( );
	try{
		return recurseDirReturnObj( "/proc/sys/" );
...
function sysctlSet( path, value ){
	console.log( "DEBUG: got path of '" + path + "' and value of '" + value + "'" );
}

function recurseDirReturnObj( directoryToRecurse ){
	var returnArray	= Array( );
	try{
		...
		for( var x=0; x<directoryContents.length; x++ ){
			...
			if( tmpStatObj.isDirectory() ){
				returnArray[directoryContents[x].replace( RegExp( '-', 'g' ), '_' )]	= recurseDirReturnObj( path.join( directoryToRecurse, directoryContents[x] ) );
			}else{
				// Open the file for reading..
				var tmpFileContent	= fs.readFileSync( path.join( directoryToRecurse, directoryContents[x] ), 'utf8' );
				returnArray[directoryContents[x].replace( RegExp( '-', 'g' ), '_' )] = function( args ){
					if( !args ){
						return tmpFileContent.replace( RegExp( '\n$', 'g' ), '' );
					}else{
						// Make a call to a setter here..
					}
				};
			}
		}
